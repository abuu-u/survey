...
export function copyStatic() {
  return src(PATHS_TO_STATIC, { base: PATH_TO_SOURCE }).pipe(dest(PATH_TO_DIST))
}
...
export function startServer() {
  ...
  server.init(
    ...
    (error, bs) => {
      bs.addMiddleware('*', (request, response) => {
        response.write(readFileSync(`${PATH_TO_DIST}/404.html`))
        response.end()
      })
    },
  )
